...
extern float calculateAverageVolume(float* samples, int blockSize);
extern float findReferenceVolume(float* samples, int blockSize);
extern void computeAmplificationFactors(float* blockAverageVolumes, float referenceVolume, float* amplificationFactors, int numBlocks);
extern void applyAmplificationFactor(float* samples, float amplificationFactor, int blockSize);
...
int main(void)
{
	...
	while (processed_bytes < chunk_size)
	{
		...
		else if (strcmp(item, "data") == 0)
		{
			...
			float* left_channel_average_volumes = (float*)calloc(num_blocks, sizeof(float));
			float* right_channel_average_volumes = (float*)calloc(num_blocks, sizeof(float));
			...
			for (int i = 0; i < num_blocks; i++) {
				// Calculate the start sample(index) for the current block
				int start_sample = i * block_size;

				// Calculate the average volume for the current block
			...
			// Allocate memory for amplification factors
			...
			for (int i = 0; i < num_blocks; i++) {
				...
				// Apply amplification factor to the current block
